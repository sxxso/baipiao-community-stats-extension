...
资金记录、最近活动和毛排行榜都是通过白嫖社区的接口直接读取的，在任何社区页面点刷新都能获取，不需要先打开个人主页、资金记录页或排行榜页，也不需要等待页面额外加载。毛余额刷新时优先读取 `/bbs/api/users/<用户ID>` 的 `money` 字段；资金流水接口只提供交易行，管理员调账等没有流水的变化也会正确反映。毛排行榜读取 `/bbs/api/money-rank`，展示前若干名、自己的名次和全站有毛的账号数。用户接口暂时不可用时会回退到页面内嵌的余额，排行榜接口暂时不可用时会保留上次成功读取的榜单；整个刷新请求失败时才保留上次成功缓存。
...
插件不读取 `document.cookie`、密码输入框或网站 `localStorage`，不保存和上传密码、Cookie、Authorization、Token、完整 HTML 或原始响应。归一化后的统计快照、最多 5 条资金记录，以及毛排行榜前 10 名的公开榜单字段（排名、用户名、余额、主页链接）只保存在当前浏览器本地，不会上传到作者服务器；排行榜不保存他人头像或原始响应。
