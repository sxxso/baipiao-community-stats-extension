(function (root, factory) {
  ...
  function sanitizeCheckin(input) {
    ...
    const checkin = {
      month:
        typeof input.month === "string" && input.month.trim() ? input.month.trim() : null,
      checked: input.checked === true,
      canCheckin: input.canCheckin !== false,
      monthDays: finiteOrNull(input.monthDays),
      monthEarned: finiteOrNull(input.monthEarned),
      todayReward: finiteOrNull(input.todayReward),
      maxDaily: finiteOrNull(input.maxDaily),
      blockedReason:
        typeof input.blockedReason === "string" && input.blockedReason.trim()
          ? input.blockedReason.trim()
          : null,
      tier: sanitizeCheckinTier(input.tier),
      nextTier: sanitizeCheckinTier(input.nextTier, true),
      url: allowedUrl(input.url),
    };
    const hasContent =
      checkin.month !== null ||
      checkin.checked ||
      !checkin.canCheckin ||
      checkin.monthDays !== null ||
      checkin.monthEarned !== null ||
      checkin.todayReward !== null ||
      checkin.maxDaily !== null ||
      checkin.blockedReason !== null ||
      checkin.tier !== null ||
      checkin.nextTier !== null;
    return hasContent ? checkin : null;
  }

  function sanitizeCheckinTier(input, withDays = false) {
    if (!isObject(input)) return null;
    const tier = {
    ...
    if (withDays) tier.daysUntil = finiteOrNull(input.daysUntil);
    const hasContent = Object.values(tier).some((value) => value !== null);
    return hasContent ? tier : null;
  }
  ...
  function sanitizeSnapshot(input) {
    ...
    const checkin = sanitizeCheckin(input.checkin);
    ...
    return {
      ...
      leaderboard,
      checkin,
      quests: sanitizeQuests(input.quests),
      trend,
      fetchedAt:
