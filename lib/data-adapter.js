(function (root, factory) {
  ...
  function normalizeProfile(input, base = ORIGIN) {
    ...
    const username = text(
      firstValue(profile.username, profile.user_name, profile.handle, profile.login),
    ) || null;
    const profileUrl = sanitizeUrl(
      firstValue(profile.profileUrl, profile.profile_url, profile.url) ||
    ...
    return {
      ...
      avatarUrl: normalizeAvatar(
        firstValue(profile.avatarUrl, profile.avatar_url, profile.avatar_template, profile.avatar),
      ),
      ...
      joinedAt: dateValue(
        firstValue(profile.joinedAt, profile.joined_at, profile.createdAt, profile.created_at),
      ),
  ...
  function normalizePayload(payload, source = "api") {
    ...
    const user = objectOrEmpty(
      firstValue(body.user, body.profile, body.current_user, body.currentUser, data.user),
    );
    const summary = objectOrEmpty(
      firstValue(
        body.summary,
        body.stats,
        body.user_summary,
